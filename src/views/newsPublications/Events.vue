    ...
    <div class="widget--fantasy">
      <div class="widget--fantasy-title">
        <h1 class="m-0 text-capitalize">
          <strong v-if="english">Events</strong>
          <strong v-if="french">événements</strong>
        </h1>
      </div>
    </div>
    <!--  -->
...
export default {
  ...
  computed: {
    ...
    posts () {
      return this.newsAndPublications.filter((news) => news.type.toLowerCase().includes('events') || news.type.toLowerCase().includes('événement'))
    }
  },
  created() {
