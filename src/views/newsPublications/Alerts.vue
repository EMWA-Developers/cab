  ...
  <div class="">
    ...
    <div class="widget--fantasy">
      <div class="widget--fantasy-title">
        <h1 class="m-0">
          <strong v-if="english">Alerts</strong>
          <strong v-if="french">Alertes</strong>
        </h1>
      </div>
    </div>
    <!--  -->
...
export default {
  ...
  computed: {
    ...
    posts () {
      return this.newsAndPublications.filter((news) => news.types.find(type => type.toLowerCase().includes('alerts') || type.toLowerCase().includes('alerte')))
    }
  },
  created() {
